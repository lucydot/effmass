...
def check_partial_occupancy(occupancy):
    ...
    if occupancy.any() not in [0,1,2]:
        warnings.warn("You have partial occupation numbers in Data.occupancy. Anything with occupation > 0.2 will be considered part of the valence band, anything with occupation < 0.8 will be considered part of the conduction band.")
...
def calculate_direction(a, b):
    ...
    a = np.array(a)
    b = np.array(b)
    direction_masked = ma.masked_equal(a - b, 0) # return array with invalid entries where values are equal
    direction_filled = ma.filled(direction_masked, 10**6) # fill invalid elements of array with a large number s
    direction_absolute = np.absolute(direction_filled) # return absolute values of each element 
    smallest = np.amin(direction_absolute)
    direction = (b - a) / smallest # use the minimum absolute value as a divisor a-b
    if -1 in direction:
...
def change_direction(kpoints, kpoint_indices):
    ...
    """
    new_direction = calculate_direction(kpoints[kpoint_indices[0]], kpoints[kpoint_indices[1]])
    change_index = None
    for i,value in enumerate(kpoint_indices[:-1]):
        old_direction = new_direction 
        new_direction = calculate_direction(
                                     kpoints[kpoint_indices[i]], 
...
def find_extrema_indices(Data, Settings):
    ...
    turning points within :attr:`effmass.inputs.Settings.energy_range` of the conduction band minimum (:attr:`effmass.inputs.Data.CBM`) / valence band maximum (:attr:`effmass.inputs.Data.VBM`).
    ...
    holes_in_range = ma.masked_where(np.absolute(energy_occupied - Data.VBM) > Settings.extrema_search_depth, energy_occupied)
    hole_maxima = ma.masked_where(_mark_maxima(holes_in_range) == 0,holes_in_range)
    
    # returns array of energies within energy_range
    electrons_in_range = ma.masked_where(np.absolute(energy_unoccupied - Data.CBM) > Settings.extrema_search_depth, energy_unoccupied)
    electron_minima = ma.masked_where(_mark_minima(electrons_in_range) == 0, electrons_in_range)
    ...
    extrema_position = np.append(np.argwhere(hole_maxima.mask == 0),
                                 np.argwhere(electron_minima.mask == 0),
                                 axis=0)
    return extrema_position

def _mark_maxima(holes_array):
    """ Helper function which takes an array of hole energies and returns an array for masking those which are not maxima
    """
...
def _mark_minima(electrons_array):
    """ Helper function which takes an array of electron energies and returns an array for masking those which are not minima
    """
    # create array to append to
    not_minima = [ ]
    ...
    for band in range(0, height):
        for k in range(1, width-1):
            if (electrons_array[band,k-1] <= electrons_array[band,k] 
                or electrons_array[band,k+1] <= electrons_array[band,k]):
...
def _within(band_index, kpoint_index, trial_kpoint_index, Settings, Data):
    ...
    if (np.absolute(Data.energies[band_index,trial_kpoint_index] - 
        Data.energies[band_index,kpoint_index])) < Settings.energy_range:
        return True
    else:
        return False

def _kpoints_after(band_index, kpoint_index, Settings, Data):
    """ Helper function that returns eigenstates which are 1) on the same band 2) before the given kpoint in the bandstructure route and 3) within :attr:`effmass.inputs.Settings.energy_range`.
    """
    trial_kpoint_index = kpoint_index + 1   
    kpoint_after = [kpoint_index]                                
    while (trial_kpoint_index < Data.number_of_kpoints and 
           _within(band_index, kpoint_index, 
           trial_kpoint_index, Settings, Data) is True):
...
def _kpoints_before(band_index, kpoint_index, Settings, Data):
    """ Helper function that returns eigenstates which are 1) on the same band 2) before the given eigenstate in the route through reciprocal space 3) within :attr:`effmass.inputs.Settings.energy_range`.
    """
    trial_kpoint_index = kpoint_index -1                             
    kpoint_before = [kpoint_index]
    while trial_kpoint_index >= 0 and _within(band_index, 
            kpoint_index, trial_kpoint_index, Settings, Data) is True:
        kpoint_before.append(trial_kpoint_index)
        trial_kpoint_index = trial_kpoint_index - 1
    
    return kpoint_before
  
def get_kpoints_before(band_index, kpoint_index, Settings, Data, truncate_dir_change=True):
    """For a given eigenstate, finds eigenstates which 1) belong to the same band 2) come before the given eigenstate in the route through reciprocal space 3) are within :attr:`effmass.inputs.Settings.energy_range`.
    ...
    """
    kpoints_before =  _kpoints_before(
...
def get_kpoints_after(band_index, kpoint_index, Settings, Data, truncate_dir_change=True):
    """For a given eigenstate, finds eigenstates which 1) belong to the same band 2) come after the given eigenstate in the route through reciprocal space 3) are within :attr:`effmass.inputs.Settings.energy_range`.
...
def generate_segments(Settings, Data, truncate_dir_change=True):  
    ...
    for band, kpoint in extrema_array:
        kpoints_before = get_kpoints_before(band,kpoint,Settings, Data,truncate_dir_change=truncate_dir_change)
        kpoints_after = get_kpoints_after(band,kpoint,Settings, Data,truncate_dir_change=truncate_dir_change)
        if kpoints_before:
        ...
        if kpoints_after:
            ...
            band_list.append(band)
    segments = [analysis.Segment(Data,band,kpoints) for band,kpoints in zip(band_list,kpoints_list)]
    return segments
